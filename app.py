...
logging.basicConfig(
    ...
    handlers=[
        ...
        # Removed console StreamHandler to avoid Windows charmap/emoji issues
    ]
)
...
# Helper: ensure additional section titles are meaningful
def normalize_section_name(name: str, items) -> str:
    try:
        raw = (name or "").strip()
        # Flatten items to single lowercase text blob for heuristics
        text = " ".join([str(i) for i in (items or [])]).lower()

        # If name is generic or missing, infer from content
        generic = {"section_name", "section", "misc", "miscellaneous", "other", "additional", "untitled", "unknown"}
        if not raw or raw.replace("_", "").lower() in generic:
            # Heuristic buckets
            if any(k in text for k in ["remote", "relocation", "visa", "sponsorship", "timezone", "tz", "hybrid", "onsite", "availability"]):
                return "Work Preferences"
            if any(k in text for k in ["citizenship", "work authorization", "authorization", "clearance"]):
                return "Work Authorization"
            if any(k in text for k in ["hobby", "interests", "interest"]):
                return "Interests"
            if any(k in text for k in ["award", "honor", "recognition"]):
                return "Awards"
            if any(k in text for k in ["publication", "paper", "journal", "conference"]):
                return "Publications"
            # Fallback
            return "Additional Information"

        # Otherwise, tidy the provided name
        pretty = raw.replace("_", " ").strip()
        # Title-case but preserve all-caps words
        return pretty if pretty.isupper() else pretty.title()
    except Exception:
        return "Additional Information"

# Helper: normalize/rename additional section keys on parsed resume
def normalize_additional_sections(parsed):
    try:
        # Object-style parsed resume
        if hasattr(parsed, 'additional_sections') and isinstance(getattr(parsed, 'additional_sections', None), dict):
            normalized = {}
            for key, items in parsed.additional_sections.items():
                new_key = normalize_section_name(key, items)
                if new_key in normalized:
                    # Merge lists if duplicate after normalization
                    existing = normalized.get(new_key) or []
                    to_add = items or []
                    normalized[new_key] = list(existing) + [i for i in to_add if i not in existing]
                else:
                    normalized[new_key] = items
            parsed.additional_sections = normalized

        # Dict-style parsed resume
        if isinstance(parsed, dict) and isinstance(parsed.get('additional_sections'), dict):
            normalized = {}
            for key, items in parsed['additional_sections'].items():
                new_key = normalize_section_name(key, items)
                if new_key in normalized:
...
def _cleanup_stale_template_results():
    """Clean up stale template matching results to prevent MediaFileStorageError."""
    try:
        if 'template_matching_results' in st.session_state:
            results = st.session_state.template_matching_results
            if isinstance(results, dict) and 'iterations' in results:
                # Check if any iteration has image_bytes that might be stale
                iterations = results.get('iterations', [])
                cleaned_iterations = []
                
                for iteration in iterations:
                    # Create a copy without image_bytes to prevent MediaFileStorageError
                    if hasattr(iteration, 'image_bytes'):
                        # Remove the image_bytes to prevent stale references
                        iteration.image_bytes = None
                    cleaned_iterations.append(iteration)
                
                # Update with cleaned iterations
                results['iterations'] = cleaned_iterations
                st.session_state.template_matching_results = results
                
    except Exception as e:
        logger.warning(f"Error cleaning up template results: {e}")
        # If cleanup fails, remove the results entirely to prevent errors
        if 'template_matching_results' in st.session_state:
            del st.session_state.template_matching_results

def get_country_resume_standards(country: str) -> dict:
    """Get country-specific resume standards and requirements."""
    standards = {
        "🇫🇷 France": {
            "image_recommended": True,
            "image_requirement": "Mandatory",
            "image_recommendation": "Professional headshot is mandatory for French resumes (CV). Include a high-quality, recent photo.",
            "specific_requirements": [
                "Include professional photo (headshot) in top-right corner",
                "Use formal structure: État civil (personal info), Formation (education), Expérience (experience)",
                "Include full personal details: age, marital status, nationality (optional but common)",
                "Use chronological format (most recent first)",
                "Keep to 1-2 pages maximum",
                "Include handwritten cover letter (lettre de motivation) if specifically requested"
            ],
            "format_style": "formal_european",
            "length": "1-2 pages",
            "photo_placement": "top-right"
        },
        "🇩🇪 Germany": {
            "image_recommended": True,
            "image_requirement": "Strongly recommended",
            "image_recommendation": "Professional photo strongly recommended for German resumes (Lebenslauf).",
            "specific_requirements": [
                "Include professional photo (Bewerbungsfoto) - usually top-right",
                "Use detailed personal information section",
                "Include exact dates for all positions and education",
                "Use reverse chronological order",
                "Include language proficiency levels (A1-C2 scale)",
                "2-3 pages acceptable for detailed experience",
                "Sign and date the resume"
            ],
            "format_style": "detailed_european",
            "length": "2-3 pages",
            "photo_placement": "top-right"
        },
        "🇬🇧 United Kingdom": {
            "image_recommended": False,
            "image_requirement": "Not recommended",
            "image_recommendation": "Photos are generally not included in UK CVs unless specifically requested (e.g., modeling, acting).",
            "specific_requirements": [
                "No personal photo (anti-discrimination laws)",
                "Include personal statement/summary at the top",
                "Use reverse chronological format",
                "Include education details (especially recent graduates)",
                "2-3 pages standard length",
                "Include volunteer work and interests section",
                "Use British English spelling and terminology"
            ],
            "format_style": "british_professional",
            "length": "2-3 pages",
        ...
        "🇨🇦 Canada": {
            "image_recommended": False,
            "image_requirement": "Not recommended",
            "image_recommendation": "Photos are not included in Canadian resumes due to anti-discrimination practices.",
            "specific_requirements": [
                "No personal photo or personal details (age, marital status)",
                "Focus on skills and achievements",
                "Use Canadian English spelling",
                "Include volunteer experience prominently",
                "1-2 pages preferred",
                "Highlight bilingual capabilities (English/French) if applicable",
                "Include relevant certifications and licenses"
            ],
            "format_style": "north_american",
            "length": "1-2 pages",
            "photo_placement": "none"
        },
        "🇦🇺 Australia": {
            "image_recommended": False,
            "image_requirement": "Not recommended",
            "image_recommendation": "Photos are not typically included in Australian resumes.",
            "specific_requirements": [
                "No personal photo or personal information",
                "Use Australian English spelling",
                "Include a professional summary",
                "Highlight key achievements with metrics",
                "2-3 pages acceptable",
                "Include referees or 'References available upon request'",
                "Emphasize teamwork and cultural fit"
            ],
            "format_style": "australian_professional",
            "length": "2-3 pages",
            "photo_placement": "none"
        },
        "🇳🇱 Netherlands": {
            "image_recommended": True,
            "image_requirement": "Common practice",
            "image_recommendation": "Professional photo is commonly included in Dutch CVs.",
            "specific_requirements": [
                "Professional photo typically included",
            ...
            "format_style": "dutch_european",
            "length": "2 pages",
            "photo_placement": "top-area"
        },
        "🇸🇪 Sweden": {
            "image_recommended": False,
            "image_requirement": "Optional",
            "image_recommendation": "Photo is optional in Swedish CVs - include only if relevant to the position.",
            "specific_requirements": [
                "Photo optional (include only if job-relevant)",
                "Include personal information selectively",
                "Emphasize work-life balance and sustainability",
                "Use clean, minimalist design",
                "1-2 pages preferred",
                "Include language skills",
                "Highlight international experience"
            ],
            "format_style": "scandinavian_minimal",
            "length": "1-2 pages",
            "photo_placement": "optional-top"
        },
        "🇪🇸 Spain": {
            "image_recommended": True,
            "image_requirement": "Commonly expected",
            "image_recommendation": "Professional photo is commonly included in Spanish CVs.",
            "specific_requirements": [
                "Include professional photo",
                "Include personal details (age, marital status common)",
                "Use chronological format",
                "Include language certificates and levels",
                "2 pages standard",
                "Include computer skills section",
                "Highlight international experience"
            ],
            "format_style": "spanish_european",
            "length": "2 pages",
            "photo_placement": "top-right"
        },
        "🇮🇹 Italy": {
            "image_recommended": True,
            "image_requirement": "Expected",
            "image_recommendation": "Professional photo is expected in Italian CVs (Curriculum Vitae).",
            "specific_requirements": [
                "Include professional photo",
                "Include detailed personal information",
                "Use Europass format or similar structure",
                "Include language certificates (CEFR levels)",
                "2-3 pages acceptable",
                "Include computer skills and certifications",
                "Sign and date the CV"
            ],
            "format_style": "italian_european",
            "length": "2-3 pages",
            "photo_placement": "top-left"
        },
        "🇯🇵 Japan": {
            "image_recommended": True,
            "image_requirement": "Mandatory",
            "image_recommendation": "Professional photo is mandatory for Japanese resumes (rirekisho). Use formal business attire.",
            "specific_requirements": [
                "Include professional photo in business attire",
                "Use standardized rirekisho format",
                "Include detailed personal information",
                "Write in chronological order (oldest first - unlike Western resumes)",
                "Include family composition if culturally appropriate",
                "Handwritten applications may be preferred for traditional companies",
                "Include club activities and volunteer work"
            ],
            "format_style": "japanese_traditional",
            "length": "1-2 pages",
            "photo_placement": "designated-area"
        },
        "🇰🇷 South Korea": {
            "image_recommended": True,
            "image_requirement": "Expected",
            "image_recommendation": "Professional photo is expected in Korean resumes. Use formal business attire.",
            "specific_requirements": [
                "Include professional photo in business attire",
                "Include detailed personal information (age, gender, address)",
                "Use formal, respectful language",
                "Include education details prominently",
                "List military service (for men)",
                "Include language proficiency scores (TOEIC, etc.)",
                "2 pages standard"
            ],
            "format_style": "korean_formal",
            "length": "2 pages",
            "photo_placement": "top-area"
        },
        "🇧🇷 Brazil": {
            "image_recommended": False,
            "image_requirement": "Not recommended",
            "image_recommendation": "Photos are not recommended in Brazilian resumes due to anti-discrimination practices.",
            "specific_requirements": [
                "No personal photo or personal details (age, marital status)",
                "Use reverse chronological format",
                "Include detailed education information",
                "Highlight language skills (English proficiency important)",
                "1-2 pages preferred",
                "Include relevant courses and certifications",
                "Use Brazilian Portuguese terminology"
            ],
            "format_style": "brazilian_professional",
            "length": "1-2 pages",
            "photo_placement": "none"
        },
        "🇲🇽 Mexico": {
            "image_recommended": True,
            "image_requirement": "Commonly included",
            "image_recommendation": "Professional photo is commonly included in Mexican resumes.",
            "specific_requirements": [
                "Professional photo commonly included",
            ...
            "format_style": "mexican_traditional",
            "length": "2 pages",
            "photo_placement": "top-right"
        }
    }
    
    result = standards.get(country, {
        "image_recommended": False,
        "image_requirement": "Not specified",
        "image_recommendation": "Follow local standards for your target country.",
        "specific_requirements": ["Use professional formatting", "Keep content relevant and concise"],
        "format_style": "generic",
        "length": "1-2 pages",
        "photo_placement": "none"
    })
    
    # Add the country name to the result for translation guidance
...
def initialize_session_state():
    ...
    # Clean up stale template matching results to prevent MediaFileStorageError
    _cleanup_stale_template_results()
    ...
    # Initialize company research state
    if 'company_research_data' not in st.session_state:
        st.session_state.company_research_data = None
    if 'company_research_name' not in st.session_state:
        st.session_state.company_research_name = None
    if 'formatted_company_research' not in st.session_state:
        st.session_state.formatted_company_research = None
...
\\end{document}""",
    ...
    if latex_content.strip():
        if st.button("🚀 Process LaTeX Code", type="primary", width="stretch"):
    ...
    try:
        with st.spinner("🔄 Analyzing your resume..."):
            ...
            if success:
                ...
                st.session_state.parsed_resume = normalize_additional_sections(st.session_state.parsed_resume)
...
def display_parsed_resume_content():
    ...
    parsed = normalize_additional_sections(parsed)
...
def display_latex_code():
    ...
    if st.session_state.openrouter_client:
        ...
        with col1:
            if st.button("🔧 Improve Resume Content", width="stretch"):
                improve_resume_content_with_comparison()
        
        with col2:
            if st.button("📊 Detailed Critique", width="stretch"):
                show_detailed_critique()
        
        with col3:
            if st.button("🔄 Regenerate LaTeX", width="stretch", help="Regenerate LaTeX from parsed resume data"):
                regenerate_latex_from_parsed_data()
        
        with col4:
            with st.popover("🎯 Quick Job Adapt", width="stretch"):
        ...
        with st.spinner("📊 Analyzing resume in detail using smartest AI parsing..."):
            ...
            with st.expander("📊 **DETAILED RESUME CRITIQUE REPORT**", expanded=True):
                ...
                if st.session_state.openrouter_client:
                    ...
                    if st.button("🚀 Get AI Recommendations", width="stretch"):
...
def display_download_options():
    ...
    with col1:
        ...
        # ZIP package
        if st.button("📦 Create ZIP Package", width="stretch"):
            create_and_download_zip()
    
    with col2:
        st.markdown("#### 📄 Compiled PDF")
        
        if st.button("🚀 Compile to PDF", type="primary", width="stretch"):
...
"""
            ...
            if adapted_content and not adapted_content.startswith("Error:"):
                ...
                st.session_state.parsed_resume = sanitize_data(adapted_parsed_resume)
                st.session_state.parsed_resume = normalize_additional_sections(st.session_state.parsed_resume)  # Update current resume
...
def generate_html_with_loading(template_changed=False, palette_changed=False, role_changed=False):
    ...
    try:
        ...
        if hasattr(st.session_state, 'keep_original_layout') and st.session_state.keep_original_layout:
            ...
            # Get country standards, profile image, and translation setting from session state
            country_standards = None
            profile_image_data = None
            translate_content = getattr(st.session_state, 'translate_content', False)
            
            if hasattr(st.session_state, 'target_country') and st.session_state.target_country != "None (Generic/US)":
                country_standards = get_country_resume_standards(st.session_state.target_country)
            
            if hasattr(st.session_state, 'profile_image'):
                profile_image_data = st.session_state.profile_image
...
def display_integrated_template_matching():
        ...
        if uploaded_template:
            try:
                ...
                if target_template_image:
                    ...
                    with col2:
                        try:
                            st.image(target_template_image, caption="Template Reference", width=None)
                        except Exception as img_error:
                            st.warning(f"⚠️ Could not display template preview: {str(img_error)}")
                            logger.warning(f"Template image display error: {img_error}")
            except Exception as e:
    ...
    if not st.session_state.template_matching_in_progress:
        if st.button("🚀 Start Template Matching", type="primary", width="stretch"):
                    ...
                    if results.get('success') and results.get('final_html'):
                        ...
                        if '<style>' in improved_html and '</style>' in improved_html:
                            ...
                            if results.get('iterations'):
                                ...
                                st.dataframe(iterations_data, width="stretch")
    ...
    if 'template_matching_results' in st.session_state and st.session_state.template_matching_results:
        ...
        with st.expander("🔍 View Detailed Results", expanded=False):
            ...
            if results.get('iterations'):
                ...
                st.dataframe(iteration_data, width="stretch")
    ...
    with col2:
        with st.expander("🔍 API Call Logs (Live)"):
            if st.session_state.get('show_api_logs', False):
                ...
                if st.button("🧹 Clear Session Logs", width="stretch"):
...
def improve_section_with_prompt(comparison_service, section_key, prompt):
    ...
    try:
        ...
        st.error(f"âŒ Error improving section: {str(e)}")
...
def show_detailed_critique():
    ...
    try:
        ...
        st.error(f"âŒ Error generating critique: {str(e)}")
        logger.error(f"Critique generation error: {e}")
    ...
    try:
        with st.spinner("🤖 Generating AI enhancement recommendations..."):
            ...
            if recommendations and not recommendations.startswith("Error:"):
                ...
                st.error("âŒ Failed to generate AI recommendations")
                
    except Exception as e:
        st.error(f"âŒ Error getting AI recommendations: {str(e)}")
    ...
    try:
        with st.spinner("✨ Applying AI improvements..."):
            ...
            if improved_latex and not improved_latex.startswith("Error:"):
                ...
                st.error("âŒ Failed to apply improvements")
                
    except Exception as e:
        st.error(f"âŒ Error applying improvements: {str(e)}")
    ...
    try:
        with st.spinner("🔄 Regenerating LaTeX from parsed resume data..."):
            if st.session_state.parsed_resume:
                ...
                st.error("âŒ No parsed resume data available. Please analyze a resume first.")
                
    except Exception as e:
        st.error(f"âŒ Error regenerating LaTeX: {str(e)}")
        logger.error(f"LaTeX regeneration error: {e}")
    ...
    if st.button("📄 Compile & Download PDF", type="primary", width="stretch"):
        with st.spinner("🔄 Compiling LaTeX to PDF..."):
            ...
            if success and pdf_bytes:
                ...
                st.error(f"âŒ Compilation failed: {message}")
            ...
            if sections_data:
                ...
                if st.session_state.openrouter_client:
                    ...
                    if enhanced_tex and not enhanced_tex.startswith("Error:"):
                        ...
                        st.error("âŒ AI enhancement failed. Please try the 'Improve LaTeX Code' button instead.")
                else:
                    st.error("âŒ Please enter your OpenRouter API key to use enhanced generation.")
            else:
                st.error("âŒ No analysis data available for enhancement.")
                
        except Exception as e:
            st.error(f"âŒ Enhanced generation failed: {str(e)}")
            logger.error(f"Enhanced generation error: {e}")

def adapt_resume_to_job(job_description: str, adaptation_level: str = "moderate"):
    """Adapt resume to specific job description with comparison interface."""
    if not st.session_state.openrouter_client:
        st.error("âŒ Please enter your OpenRouter API key for job adaptation.")
        return
    
    if not st.session_state.parsed_resume or not st.session_state.original_parsed_resume:
        st.error("âŒ No parsed resume data available for adaptation.")
        return
            ...
            if adapted_content and not adapted_content.startswith("Error:"):
                ...
                if st.button("🔄 View Detailed Before/After Comparison", type="primary", key="job_adapt_comparison"):
                    if st.session_state.enhancement_comparison and st.session_state.enhancement_comparison.comparisons:
                        ...
                        st.error("âŒ No comparison data available. Please try the adaptation again.")
                    
            else:
                st.error(f"âŒ Job adaptation failed: {adapted_content}")
                
        except Exception as e:
            st.error(f"âŒ Job adaptation error: {str(e)}")
            logger.error(f"Job adaptation error: {e}")
...
"""
        ...
        st.error(f"âŒ Error creating ZIP package: {str(e)}")
...
def create_footer():
    ...
    st.markdown("""
    <div style="text-align: center; color: #7f8c8d; padding: 2rem 0;">
        <p>Built with â¤ï¸ using Streamlit • Resume to LaTeX Generator</p>
...
def auto_generate_html_preview():
    ...
    try:
        ...
        # Generate with default settings (keep original layout by default)
        logger.info(f"ðŸ­ Calling generate_original_format_html for auto-preview")
        html_result = st.session_state.html_generator.generate_original_format_html(
            resume_dict,
            original_analysis=getattr(st.session_state, 'resume_analysis', None),
            enhancement_level="minimal",
        ...
        logger.error(f"âŒ Auto HTML generation error: {e}")
        logger.error(f"📊 Auto-preview error traceback:", exc_info=True)
...
def generate_html_with_loading(template_changed=False, palette_changed=False, role_changed=False):
    ...
    try:
        ...
        # Log session state update with detailed information
        logger.info(f"ðŸ’¾ UPDATING SESSION STATE - st.session_state.html_resume_content")
        logger.info(f"📄 HTML result keys: {list(html_result.keys()) if isinstance(html_result, dict) else 'Not a dict'}")
            ...
            if target_role != "None (Original)" and resume_dict.get('_role_adapted'):
                ...
                if adapted_summary and len(adapted_summary) > 50:
                    ...
                    if summary_sample in html_content.lower():
                        ...
                        logger.error(f"âŒ SESSION STATE: Adapted summary NOT FOUND in new HTML content")
                        logger.debug(f"Looking for summary: {summary_sample}...")
        ...
        loading_placeholder.empty()
        st.error(f"âŒ Error generating HTML resume: {str(e)}")
        logger.error(f"HTML generation error: {e}")
        logger.error(f"HTML generation error details: {type(e).__name__}: {str(e)}")
        import traceback
        logger.error(f"HTML generation traceback: {traceback.format_exc()}")
...
def display_integrated_template_matching():
    ...
    if template_option == "Upload Custom Template Image":
        ...
        if uploaded_template:
            try:
                ...
                st.error(f"âŒ Failed to process template image: {e}")
                return
    else:
        # Use original PDF - we'll extract it in the matching process
        st.info("✅ Will use your original resume PDF as template reference")
    ...
    try:
        ...
        if selected_model_info:
            ...
            try:
                pricing_float = float(pricing) if pricing else 0
                is_free = pricing_float == 0
                cost_info = "🏆“ Free" if is_free else f"ðŸ’° ${pricing_float:.4f}/1k tokens"
            except (ValueError, TypeError):
                cost_info = "ðŸ’° Paid model"
        ...
        if st.button("🚀 Start Template Matching", type="primary", width="stretch"):
            ...
            with st.spinner("🔄 Analyzing template and improving layout..."):
                try:
                    ...
                    st.error(f"âŒ Template matching error: {str(e)}")
                finally:
                    st.session_state.template_matching_in_progress = False
                    st.rerun()
    else:
    ...
    if 'template_matching_results' in st.session_state and st.session_state.template_matching_results:
        ...
        with st.expander("🔍 View Detailed Results", expanded=False):
            ...
            with col2:
                st.metric("Final Score", f"{results.get('final_score', 0)}%")
                st.metric("Target Reached", "✅ Yes" if results.get('target_reached') else "âŒ No")
...
def display_template_matching_interface():
            ...
            try:
                ...
                if results.get('success'):
                    ...
                    if final_html:
                        ...
                        # Apply confirmation section
                        st.markdown("---")
                        st.markdown("### ðŸ¤” Apply Template Matching?")
                        ...
                        with col_action:
                            ...
                            if st.button("✅ Apply Template Matching", key="apply_template_match", help="Apply the template matched resume as your current resume"):
                                # Apply the template matched resume
                                st.session_state.html_resume_content = final_html
                                st.success("ðŸŽ‰ Template matching applied successfully!")
                                st.info("💡 Your resume preview has been updated with the new template matching!")
                                st.rerun()
                            
                            if st.button("âŒ Keep Current", key="reject_template_match", help="Keep your current resume and discard template matching"):
                                st.info("ðŸ‘ Keeping your current resume. Template matching results discarded.")
                ...
                st.error(f"âŒ Error during template matching: {str(e)}")
                logger.error(f"Template matching error: {e}")
    ...
    with col1:
        with st.expander("📝‹ Dependencies Information"):
            ...
            st.markdown("**Image Conversion Methods:**")
            for method, available in dependencies.items():
                status = "✅ Available" if available else "âŒ Not Available"
                st.markdown(f"- **{method}**: {status}")
    ...
    with col2:
        with st.expander("🔍 API Call Logs (Live)"):
            ...
            st.markdown("**Logging includes:**")
            st.markdown("- 🤖 Model used")
            st.markdown("- 📊 Token counts (input â†’ output)")
            st.markdown("- â±ï¸ Response time")
            st.markdown("- ✅/âŒ Success/failure status")
...
def display_html_resume_builder():
    ...
    if st.session_state.html_resume_content:
        ...
        # Country Standards & Profile Image Section
        st.markdown("### 🌍 Country Standards & Profile Image")
        st.markdown("*Generate resumes optimized for specific countries and include professional photos*")
        
        col_country1, col_country2, col_country3 = st.columns([2, 2, 1])
        
        with col_country1:
            # Country selection
            available_countries = [
                "None (Generic/US)", "Germany", "France", "United Kingdom", "Canada", 
                "Australia", "Netherlands", "Switzerland", "Sweden", "Denmark",
                "Norway", "Belgium", "Austria", "Ireland", "New Zealand"
            ]
            
            current_country = getattr(st.session_state, 'target_country', "None (Generic/US)")
            target_country = st.selectbox(
                "🌍 Target Country:",
                available_countries,
                index=available_countries.index(current_country) if current_country in available_countries else 0,
                help="Select country for resume format optimization and standards compliance"
            )
            st.session_state.target_country = target_country
            
            # Show country-specific info
            if target_country != "None (Generic/US)":
                standards = get_country_resume_standards(target_country)
                if standards:
                    st.success(f"✅ {target_country} standards will be applied")
                    if standards.get('requires_photo'):
                        st.info(f"📷 {target_country} commonly includes profile photos")
        
        with col_country2:
            # Profile image upload
            st.markdown("**📷 Profile Image (Optional)**")
            ...
            if uploaded_image:
                try:
                    ...
                    # Show image preview
                    st.image(uploaded_image, width=100, caption="Profile Preview")
                    
                except Exception as e:
                    st.error(f"❌ Error uploading image: {e}")
                    if 'profile_image' in st.session_state:
                        del st.session_state.profile_image
            else:
                if 'profile_image' in st.session_state:
                    del st.session_state.profile_image
        ...
        with col_country3:
            ...
            translate_content = st.checkbox(
                "🌐 Translate Content",
                value=getattr(st.session_state, 'translate_content', False),
                help="Adapt language and expressions for the target country"
            )
            st.session_state.translate_content = translate_content
            
            # Apply button for regeneration
            apply_changes = st.button(
                "🔄 Apply Changes",
                type="primary",
                width="stretch",
                help="Regenerate resume with country standards, profile image, and translation settings"
            )
            
            if apply_changes:
                with st.spinner("🔄 Applying country standards and settings..."):
                    # Get country standards
                    country_standards = None
                    if target_country != "None (Generic/US)":
                        country_standards = get_country_resume_standards(target_country)
                    
                    # Get profile image
                    profile_image_data = getattr(st.session_state, 'profile_image', None)
                    
                    # Get translation preference
                    translation_enabled = translate_content
                    
                    # Regenerate HTML with new settings
                    try:
                        resume_dict = convert_parsed_resume_to_dict(st.session_state.parsed_resume)
                        html_result = st.session_state.html_generator.generate_resume_html(
                            resume_dict,
                            ...
                            country_standards=country_standards,
                            profile_image_data=profile_image_data,
                            translate_content=translation_enabled
                        )
                        st.session_state.html_resume_content = html_result
                        st.success("✅ Resume regenerated with new settings!")
                        st.rerun()
                    except Exception as e:
        ...
        with tab2:
            ...
            if updated_resume_dict != resume_dict:
                ...
                if st.button("🔄 Update Preview", key="update_preview_from_edit"):
                    with st.spinner("Updating preview..."):
                        # Get country standards and profile image from session state
                        country_standards = None
                        profile_image_data = None
            ...
            if st.session_state.openrouter_client:
                ...
                if st.button("✨ Apply Custom AI Improvements", width="stretch", type="primary"):
        ...
        with tab3:
            ...
            if 'last_ats_score' in st.session_state:
                ...
                if st.session_state.openrouter_client and st.session_state.last_ats_score.overall_score < 90:
                    ...
                    with col2:
                        if st.button("✨ Apply ATS Improvements", width="stretch", type="primary"):
...
{st.session_state.html_resume_content['html']}
                ...
                with col2:
                    if st.button("🎨 Apply Color Theme", width="stretch") and color_description:
        ...
        with tab7:
            ...
            company_name = st.text_input(
                "Company Name", 
                value=getattr(st.session_state, 'cover_company', ""),
                help="Enter company name. Leave empty if the company is anonymous/confidential."
            )
            
            # Show helpful info for empty company name and company research preview
            if not company_name or not company_name.strip():
                st.info("💡 No company name provided. The cover letter will use a professional greeting and focus on role requirements without company-specific research.")
            else:
                # Show research status
                research_data = st.session_state.get('company_research_data')
                if research_data:
                    st.success(f"🏢 Company research: ✅ {len(research_data)} sources found")
                elif st.session_state.get('cover_letter_text'):
                    st.warning("🏢 Company research: ⚠️ No data collected")
                else:
                    st.info("🏢 Company research: 🔍 Generate cover letter to start research")
            
            job_desc = st.text_area("Job Description", value=getattr(st.session_state, 'cover_job_desc', ""), height=160)

            # Persist inputs and clear research data if company changed
            if st.session_state.get('cover_company') != company_name:
                # Company name changed, clear old research data
                st.session_state.company_research_data = None
                st.session_state.company_research_name = None  
                st.session_state.formatted_company_research = None
            ...
            col_length, col_char_limit = st.columns(2)
            with col_length:
                length = st.selectbox("Length", ["Short", "Medium", "Detailed"], index=1, help="Controls letter word target")
                word_target = 400 if length == "Medium" else (200 if length == "Short" else 700)
            
            with col_char_limit:
                char_limit_enabled = st.checkbox("Character Limit", help="Enforce strict character limit")
                char_limit = st.number_input(
                    ...
                    value=2000, 
                    step=100, 
                    disabled=not char_limit_enabled,
                    help="Maximum characters including spaces"
                ) if char_limit_enabled else None

            st.caption("Uses DuckDuckGo to fetch top pages (About/Overview/Careers) and attach raw HTML excerpts.")
                ...
                if st.button("Generate Cover Letter", width="stretch"):
                    if not getattr(st.session_state, 'openrouter_client', None):
                        ...
                        with st.spinner("Composing cover letter..."):
                            ...
                            # Get cover letter with research data
                            letter, research_data = st.session_state.cover_letter_service.generate_cover_letter(
                                resume_dict,
                                job_desc,
                                company_name,
                                None,
                                ...
                                ddg_only=True,
                                attach_pages=3,
                                return_research_data=True
                            )
                            
                            st.session_state.cover_letter_text = letter
                            
                            # Store research data for preview
                            if research_data:
                                st.session_state.company_research_data = research_data
                                st.session_state.company_research_name = company_name
                            
                            st.success("Draft ready. You can edit below.")

            # Research preview (DuckDuckGo-only flow attaches HTML directly in the prompt)

            # Editor and preview
            st.markdown("#### Draft")
            
            # Character limit enforcement in editor
            current_text = st.session_state.cover_letter_text or ""
            if char_limit_enabled and char_limit:
                char_count = len(current_text)
                remaining_chars = max(0, char_limit - char_count)
                
                # Show character counter
                if char_count > char_limit:
                    st.error(f"⚠️ Character limit exceeded: {char_count}/{char_limit} characters ({char_count - char_limit} over limit)")
                elif remaining_chars < 50:
                    st.warning(f"📊 Characters: {char_count}/{char_limit} ({remaining_chars} remaining)")
                else:
                    st.info(f"📊 Characters: {char_count}/{char_limit} ({remaining_chars} remaining)")
                
                # Trim text if over limit
                if char_count > char_limit:
                    current_text = current_text[:char_limit]
                    st.session_state.cover_letter_text = current_text
                    st.rerun()
            ...
            st.session_state.cover_letter_text = st.text_area(
                ...
                value=current_text,
                height=260,
                help=f"Maximum {char_limit} characters" if char_limit_enabled and char_limit else None
            )
            if st.session_state.cover_letter_text:
                ...
                with col_preview:
                    st.markdown("#### Preview")
                    # Ensure proper markdown rendering for cover letter
                    cover_text = st.session_state.cover_letter_text
                    if cover_text:
                        st.markdown(cover_text, unsafe_allow_html=False)
                
                with col_stats:
                    st.markdown("#### Stats")
                    final_char_count = len(st.session_state.cover_letter_text)
                    word_count = len(st.session_state.cover_letter_text.split())
                    st.metric("Characters", final_char_count)
                    st.metric("📋 Words", word_count, help="Current word count")
                    if char_limit_enabled and char_limit:
                        compliance = "✅ Within limit" if final_char_count <= char_limit else "❌ Over limit"
                        st.metric("Limit Status", compliance)
            ...
            # Company Research Preview Section
            if company_name and company_name.strip():
                with st.expander("🏢 Company Research Preview", expanded=False):
                    # Refresh button
                    if st.button("🔄 Reformat Research Data", key="reformat_research", help="Re-process with AI"):
                        research_data = st.session_state.get('company_research_data')
                        if research_data:
                            with st.spinner("Formatting company research with AI..."):
                                formatted_research = st.session_state.cover_letter_service.format_company_research(
                                    st.session_state.get('company_research_name', company_name),
                                    research_data
                                )
                                st.session_state.formatted_company_research = formatted_research
                    
                    # Display formatted research or no data message
                    research_data = st.session_state.get('company_research_data')
                    if research_data:
                        if st.session_state.get('formatted_company_research'):
                            st.markdown("### 📊 AI-Formatted Company Information")
                            # Ensure proper markdown rendering
                            formatted_content = st.session_state.formatted_company_research
                            if formatted_content:
                                st.markdown(formatted_content, unsafe_allow_html=False)
                        else:
                            # Auto-format on first display
                            with st.spinner("Formatting company research with AI..."):
                                formatted_research = st.session_state.cover_letter_service.format_company_research(
                                    st.session_state.company_research_name,
                                    st.session_state.company_research_data
                                )
                                st.session_state.formatted_company_research = formatted_research
                                st.markdown("### 📊 AI-Formatted Company Information")
                                if formatted_research:
                                    st.markdown(formatted_research, unsafe_allow_html=False)
                    else:
                        st.warning("### 🚫 No Company Research Data Available")
                        st.markdown("""
                        **Possible reasons:**
                        - DuckDuckGo search returned no results
                        - Network connectivity issues
                        - Company website not accessible
                        - Search queries didn't match company pages
                        
                        **Check the logs for detailed debugging information.**
                        """)
                        
                        if st.session_state.get('company_research_name'):
                            st.info(f"**Searched for:** {st.session_state.company_research_name}")
                        
                        # Manual search suggestions
                        if st.button("🔄 Try Manual Research", help="Get manual research tips"):
                            st.markdown("""
                            **Manual Research Tips:**
                            1. Search directly: `[Company Name] about`
                            2. Check LinkedIn company page
                            3. Look for official website
                            4. Check Crunchbase or similar directories
                            5. Review recent news articles
                            """)
                            
                        # Allow manual data input
                        manual_research = st.text_area(
                            "Manual Company Information",
                            help="Paste company information you found manually",
                            placeholder="Paste company description, mission, values, recent news, etc."
                        )
                        
                        if manual_research and st.button("Format Manual Data"):
                            # Create fake research data structure for manual input
                            fake_research_data = [("Manual Input", manual_research)]
                            with st.spinner("Formatting manual research with AI..."):
                                formatted_research = st.session_state.cover_letter_service.format_company_research(
                                    st.session_state.get('company_research_name', 'Company'),
                                    fake_research_data
                                )
                                st.markdown("### 📊 AI-Formatted Manual Research")
                                st.markdown(formatted_research)
                    
                    # Raw data toggle
                    if research_data and st.toggle("Show Raw Research Data", help="View unprocessed web data"):
                        st.markdown("### 🌐 Raw Research Data")
                        for i, (url, content) in enumerate(research_data):
                            with st.expander(f"Source {i+1}: {url}"):
...
def main():
    ...
    try:
        ...
        if "MediaFileStorage" in error_type or "media file" in str(e).lower():
            st.error("⚠️ **Media File Error**: Some images may not display properly due to a session timeout. Please refresh the page or re-upload any images.")
            logger.warning(f"MediaFileStorage error caught: {e}")
            
            # Clean up all potentially problematic session state
            cleanup_keys = ['uploaded_files', 'template_matching_results', 'current_html_preview']
            for key in cleanup_keys:
                if key in st.session_state:
                    del st.session_state[key]
                    
            st.info("🔄 Session state has been cleaned. Please try your operation again.")
        else:
