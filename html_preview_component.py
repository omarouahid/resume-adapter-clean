...
class HTMLResumePreview:
    ...
    def show_preview(self, 
                    ...
                    height: int = 800) -> None:
        """
        Display HTML resume preview in Streamlit with image support.
        ...
        try:
            ...
            # Extract and display images separately if they exist
            images_found = self._extract_and_display_images(complete_html)
            
            # Create HTML without data URIs for the iframe
            clean_html = self._replace_data_uris_with_placeholders(complete_html)
            
            # Add preview-specific enhancements
            enhanced_html = self._enhance_html_for_preview(clean_html)
            ...
            if images_found:
                st.info("ℹ️ Profile images are displayed above the preview due to browser security restrictions. Images will work correctly in downloaded HTML files.")
...
"""
    ...
    def _extract_and_display_images(self, html_content: str) -> bool:
        """Extract data URI images from HTML and display them using st.image()."""
        
        # Find all data URI images in the HTML
        data_uri_pattern = r'data:image/([a-zA-Z]*);base64,([^"\'>\s]*)'
        matches = re.findall(data_uri_pattern, html_content)
        
        if not matches:
            return False
        
        st.markdown("#### 📸 Profile Images")
        st.info("Images are displayed here due to browser security restrictions. They work correctly in downloaded HTML files.")
        
        # Display each image found
    ...
    def _replace_data_uris_with_placeholders(self, html_content: str) -> str:
        """Replace data URI images with placeholder text for iframe display."""
        
        # Pattern to match data URI images
        data_uri_pattern = r'<img[^>]*src="data:image/([a-zA-Z]*);base64,([^"\']*)"[^>]*>'
        
        def replace_with_placeholder(match):
            # Extract image attributes (alt text, class, style, etc.)
            full_img_tag = match.group(0)
            
            # Extract alt text if available
            alt_match = re.search(r'alt="([^"]*)"', full_img_tag)
            alt_text = alt_match.group(1) if alt_match else "Profile Image"
            
            # Extract classes if available
            class_match = re.search(r'class="([^"]*)"', full_img_tag)
            classes = class_match.group(1) if class_match else ""
            
            # Extract style if available
            style_match = re.search(r'style="([^"]*)"', full_img_tag)
            style = style_match.group(1) if style_match else ""
            
            # Create placeholder with similar styling
            placeholder_style = f"{style}; background: #f0f0f0; display: flex; align-items: center; justify-content: center; color: #666; font-size: 12px; text-align: center; border: 2px dashed #ccc;"
            
            return f'<div class="{classes}" style="{placeholder_style}">📷<br>{alt_text}<br>(Image available in downloaded version)</div>'
        
        # Replace all data URI images with placeholders
        clean_html = re.sub(data_uri_pattern, replace_with_placeholder, html_content)
